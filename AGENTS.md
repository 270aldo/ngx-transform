...
All commands run from the repository root (`/Users/aldoolivas/genesis-scann`):
...
**Stack**: Next.js 16.2.4 (App Router) + React 19 + Firebase + Google Gemini + Tailwind CSS v4
...
| `D7Conversion` | `src/emails/sequence/` | Day 7 | NGX HYBRID conversion CTA |
...
**Fonts**: United Sans Cond (self-hosted display), Inter via `next/font` (body/UI), JetBrains Mono via `next/font` (metrics/labels)
...
| Endpoint | Protection |
|----------|------------|
| `DELETE /api/sessions/[shareId]` | Delete token validation via `X-Delete-Token` header |
